...
from utils.messages import create_conversation, save_message, get_last_messages
...
async def process_rag_pipeline(
    ...
    for document_id in documents_id:
        try:
            resp = supabase.table("documents").select("*").eq("id", document_id).eq("user_id", user_id).execute()
            if resp.data:
    ...
    await save_message(message=full_answer, conversation_id=conversation_id, user_id=user_id, role="assistant")
